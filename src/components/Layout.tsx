...
export default function Layout({ children }: LayoutProps) {
  ...
  const { showWelcomeScreen } = useDownloadSelection();
  ...
  // Listen for keyboard shortcuts only (empty state now comes from context)
  useEffect(() => {
  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden bg-background relative">
      {/* Animated Gradient Mesh Background - Only on Welcome Screen */}
      {showWelcomeScreen && (
      ...
      <div className="flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-out relative z-10">
        <Header
          ...
          isEmptyState={showWelcomeScreen}
        />
        <div className="flex-1 overflow-hidden p-3 pt-2">
          <div className={`h-full w-full rounded-2xl shadow-sm overflow-hidden transition-all duration-200 ${showWelcomeScreen ? 'bg-transparent border-0' : 'bg-card border border-border'
            }`}>
            {children}
          </div>
        </div>
